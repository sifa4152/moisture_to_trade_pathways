...
def _draw_chord_diagram(ax, df_raw, title,
    ...
    # Labels
    label_r = outer_r + ps.CHORD_LABEL_OFFSET
    for i, nd in enumerate(nodes):
        angle = mid_angles[i]
        x, y  = np.cos(angle) * label_r, np.sin(angle) * label_r
        deg   = np.degrees(angle)
        ha, rotation = ("left", deg) if -90 < deg <= 90 else ("right", deg + 180)
        label = node_labels.get(nd, str(nd)) if node_labels else str(nd)  # <-- change this
...
def make_figure(save=ps.FIGS_DIR):
    ...
    fig.subplots_adjust(left=0.01, right=0.99, top=0.93, bottom=0.01, wspace=0.05)

    _draw_chord_diagram(axes[0], atmos_df, "Atmospheric moisture network", node_labels=iso_to_name)
